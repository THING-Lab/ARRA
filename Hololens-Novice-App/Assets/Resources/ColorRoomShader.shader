...
Shader "Unlit/ColorRoomShader"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#define MAX_SIZE 100
			//TODO Set MAX_SIZE as definable shader property if possible.
			...
			v2f vert (appdata v)
			{
				...
				//transfrm model coordinate to world, because modelToWorld matrix might change if we simpy copy meshes
				//into editor, we have to memorize the matrix in the game and reuse it in the editor to construct model
				o.worlspace = mul(_MyObjectToWorld, float4(v.vertex.xyz, 1));
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				//Calculate image uv, world coordinate->camera coordinate->image coordinate
				...
				for (int j = 0; j < MAX_SIZE; ++j)
				{
					...
					if (abs(projectedTex.x) < 1.0 && abs(projectedTex.y) < 1.0 )
					{
						...
						//remap the uv, since the matrix is for original texture 1280*720, we cropped into 1024*720 to fit 
						//into texture2dArray, we need to remap the uv
						if ((unitTexcoord.x < (1024.0 / 1280.0)) && (unitTexcoord.y > (208.0 / 720.0)))
						{
							...
							//zAxis is the index of texture2dArray
